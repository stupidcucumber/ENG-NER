...
def _lowlevel_features(
    ...
    } if prefix is not None else _features
...
def _token2features(tokens: list[str], token_index: int, window_size: int) -> dict:
    ...
    _features = {
        "BOS": token_index == 0,
        "EOS": token_index == len(tokens) - 1
    }
    
    for order_index, window_token in enumerate(
        tokens[
            np.clip(token_index - window_size, 0, len(tokens)) : np.clip(token_index + window_size, 0, len(tokens))
        ],
        start=np.clip(token_index - window_size, 0, len(tokens))
    ):
        if order_index == token_index:
            _features.update(
                _lowlevel_features(token=window_token)
            )
        else:
            _features.update(
                _lowlevel_features(
...
def tokens2features(tokens: list[str], window_size: int = 2) -> list[dict]:
    ...
    """
    return [
        _token2features(tokens=tokens, token_id=token_id, window_size=window_size)
...
def numbers2labels(numbers: list[int]) -> list[str]:
    ...
    return [
        _map[number] for number in numbers
    ]
